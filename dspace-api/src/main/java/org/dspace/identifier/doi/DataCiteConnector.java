...
public class DataCiteConnector implements DOIConnector {
    ...
    /**
     * Stores the scheme used to connect to the DataCite server. It will be set by
    ...
    /**
     * Path on the DataCite server used to generate DOIs. Set by spring dependency
     * injection.
     */
    ...
    /**
     * DisseminationCrosswalk to map local metadata into DataCite metadata. The name
     * of the crosswalk is set by spring dependency injection using
    ...
    /**
     * Set the hostname of the DataCite server. Used by spring dependency injection.
     * 
     * @param DATACITE_HOST Hostname to connect to register DOIs (f.e.
     *                      test.datacite.org).
     */
    @Required
     ...
     * DataCite Metadata Schema. Used by spring dependency injection.
     * 
        ...
        if (null == this.USERNAME) {
            ...
            if (null == this.USERNAME) {
                throw new RuntimeException(
                        "Unable to load username from " + "configuration. Cannot find property " + CFG_USER + ".");
            }
        }
    ...
    protected String getPassword() {
        if (null == this.PASSWORD) {
            ...
            if (null == this.PASSWORD) {
                throw new RuntimeException(
                        "Unable to load password from " + "configuration. Cannot find property " + CFG_PASSWORD + ".");
            }
        }
    ...
    public void deleteDOI(Context context, String doi) throws DOIIdentifierException {
        ...
        // Catch all other http status code in case we forgot one.
        default: {
            log.warn("While deleting metadata of DOI {}, we got a " + "http status code {} and the message \"{}\".",
                    new String[] { doi, Integer.toString(resp.statusCode), resp.getContent() });
            throw new DOIIdentifierException(
                    "Unable to parse an answer from " + "DataCite API. Please have a look into DSpace logs.",
                    DOIIdentifierException.BAD_ANSWER);
    ...
    public void reserveDOI(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
        ...
        if (!this.xwalk.canDisseminate(dso)) {
            ...
            throw new DOIIdentifierException("Cannot disseminate " + dSpaceObjectService.getTypeText(dso) + "/"
                    + dso.getID() + " using crosswalk " + this.CROSSWALK_NAME + ".",
        ...
        if (configurationService.hasProperty(CFG_HOSTINGINSTITUTION))
            parameters.put("hostinginstitution", configurationService.getProperty(CFG_HOSTINGINSTITUTION));
        ...
        try {
            ...
            throw new DOIIdentifierException(
                    "AuthorizeException occured while " + "converting " + dSpaceObjectService.getTypeText(dso) + "/"
                            + dso.getID() + " using crosswalk " + this.CROSSWALK_NAME + ".",
            ...
            throw new DOIIdentifierException(
                    "CrosswalkException occured while " + "converting " + dSpaceObjectService.getTypeText(dso) + "/"
                            + dso.getID() + " using crosswalk " + this.CROSSWALK_NAME + ".",
        ...
        } else if (!metadataDOI.equals(doi.substring(DOI.SCHEME.length()))) 
        {
            ...
            // Copied from DATASHARE - end

            log.error("While reserving a DOI, the " + "crosswalk to generate the metadata used another DOI than "
                    + "the DOI we're reserving. Cannot reserve DOI " + doi + " for "
                    + dSpaceObjectService.getTypeText(dso) + " " + dso.getID() + ".");
            throw new IllegalStateException("An internal error occured while "
                    + "generating the metadata. Unable to reserve doi, see logs " + "for further information.");
        }
        ...
        // Catch all other http status code in case we forgot one.
        default: {
            log.warn("While reserving the DOI {}, we got a http status code " + "{} and the message \"{}\".",
                    new String[] { doi, Integer.toString(resp.statusCode), resp.getContent() });
            throw new DOIIdentifierException(
                    "Unable to parse an answer from " + "DataCite API. Please have a look into DSpace logs.",
                    DOIIdentifierException.BAD_ANSWER);
    ...
    public void registerDOI(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
        ...
        /*
         * if (!this.isDOIReserved(context, doi)) { // the DOIIdentifierProvider should
         * catch and handle this throw new
         * DOIIdentifierException("You need to reserve a DOI " +
         * "before you can register it.", DOIIdentifierException.RESERVE_FIRST); }
         */

        // send doi=<doi>\nurl=<url> to mds/doi
        DataCiteResponse resp = null;
        try {
            resp = this.sendDOIPostRequest(doi, handleService.resolveToURL(context, dso.getHandle()));
        ...
        case (400): {
            log.warn("We send an irregular request to DataCite. While " + "registering a DOI they told us: "
                    + resp.getContent());
            throw new DOIIdentifierException("Currently we cannot register "
                    + "DOIs. Please inform the administrator or take a look " + " in the DSpace log file.",
                    DOIIdentifierException.BAD_REQUEST);
        }
        ...
        case (412): {
            log.error("We tried to register a DOI {} that has not been reserved "
                    + "before! The registration agency told us: {}.", doi, resp.getContent());
            throw new DOIIdentifierException("There was an error in handling "
                    + "of DOIs. The DOI we wanted to register had not been "
                    + "reserved in advance. Please contact the administrator " + "or take a look in DSpace log file.",
                    DOIIdentifierException.RESERVE_FIRST);
        }
        // Catch all other http status code in case we forgot one.
        default: {
            log.warn("While registration of DOI {}, we got a http status code " + "{} and the message \"{}\".",
                    new String[] { doi, Integer.toString(resp.statusCode), resp.getContent() });
            throw new DOIIdentifierException(
                    "Unable to parse an answer from " + "DataCite API. Please have a look into DSpace logs.",
                    DOIIdentifierException.BAD_ANSWER);
    ...
    protected DataCiteResponse sendHttpRequest(HttpUriRequest req, String doi) throws DOIIdentifierException {
        ...
        try {
             ...
             * log.debug("Will be send to " + req.getURI().toString() + "."); if (req
             * instanceof HttpEntityEnclosingRequestBase) {
             * log.debug("Request contains entity!"); HttpEntityEnclosingRequestBase reqee =
             * (HttpEntityEnclosingRequestBase) req; if (reqee.getEntity() instanceof
             * StringEntity) { StringEntity se = (StringEntity) reqee.getEntity(); try {
             * BufferedReader br = new BufferedReader(new
             * InputStreamReader(se.getContent())); String line = null; while ((line =
             * br.readLine()) != null) { log.debug(line); } log.info("----"); } catch
             * (IOException ex) {
             * 
            ...
            case (403): {
                log.info("Managing a DOI ({}) was prohibited by the DOI " + "registration agency: {}",
                        new String[] { doi, content });
                throw new DOIIdentifierException("We can check, register or " + "reserve DOIs that belong to us only.",
                        DOIIdentifierException.FOREIGN_DOI);
            }

            // 500 is documented and signals an internal server error
            case (500): {
                log.warn("Caught an http status code 500 while managing DOI " + "{}. Message was: {}",
                        new String[] { content, doi });
                throw new DOIIdentifierException(
    ...
    protected String extractAlternateIdentifier(Context context, String content)
        ...
        try {
            ...
            throw new DOIIdentifierException("Got a JDOMException while parsing " + "a response from the DataCite API.",
                    jde, DOIIdentifierException.BAD_ANSWER);
        }
        ...
        while (handle == null && it.hasNext()) {
            ...
            try {
                handle = handleService.resolveUrlToHandle(context, alternateIdentifier.getText());
            } catch (SQLException e) {
                throw e;
            }
        }
    ...
    protected Element addDOI(String doi, Element root) {
        ...
        Element identifier = new Element("identifier",
                configurationService.getProperty(CFG_NAMESPACE, "http://datacite.org/schema/kernel-3"));
