 ...
 * 
 * <p>
 * This class handles reservation, registration and deletion of DOIs using the
 * direct API from {@link <a href="http://www.datacite.org">DataCite</a>}.
 * Please pay attention that some members of DataCite offer special services and
 * want their customers to use special APIs. If you are unsure ask your
 * registration agency.
 * </p>
 * 
 * <p>
 * Any identifier a method of this class returns is a string in the following
 * format: doi:10.123/456.
 * </p>
 * 
...
public class DOIIdentifierProvider extends IdentifierProvider {
     ...
     * registration agency needed to register DOIs. To register DOIs we have to care
     * about two APIs: the <link>IdentifierProvider</link> API of DSpace and the API
     * of the DOI registration agency. The DOIIdentifierProvider manages the DOI
     * database table, generates new DOIs, stores them as metadata in DSpace items
     * and so on. To register DOIs at DOI registration agencies it uses a
     * DOIConnector. A DOI connector has to register and reserve DOIs using the API
     * of the DOI registration agency. If requested by the registration agency it
     * has to convert and send metadata of the DSpace items.
     */
    ...
    /**
     * Part of DOI to separate several applications that generate DOIs. E.g. it
        ...
        if (null == this.PREFIX) {
            ...
            if (null == this.PREFIX) {
                ...
                throw new RuntimeException(
                        "Unable to load DOI prefix from " + "configuration. Cannot find property " + CFG_PREFIX + ".");
            }
        }
        return this.PREFIX;
    }
    ...
    public void register(Context context, DSpaceObject dso, String identifier) throws IdentifierException {
        ...
        if (DELETED.equals(doiRow.getStatus()) || TO_BE_DELETED.equals(doiRow.getStatus())) {
            throw new DOIIdentifierException("You tried to register a DOI that " + "is marked as DELETED.",
    ...
    /**
     * @param context
     * @param dso        DSpaceObject the DOI should be reserved for. Some metadata
     *                   of this object will be send to the registration agency.
     ...
     *                                  {@code dso}.
     * @see org.dspace.identifier.IdentifierProvider#reserve(Context, DSpaceObject,
     *      String)
     */
    @Override
    ...
    public void reserveOnline(Context context, DSpaceObject dso, String identifier)
        ...
        if (DELETED.equals(doiRow.getStatus()) || TO_BE_DELETED.equals(doiRow.getStatus())) {
            throw new DOIIdentifierException("You tried to reserve a DOI that " + "is marked as DELETED.",
                    DOIIdentifierException.DOI_IS_DELETED);
        }
    ...
    public void registerOnline(Context context, DSpaceObject dso, String identifier)
        ...
        // register DOI Online
        /*
         * try { connector.registerDOI(context, dso, doi); } catch
         * (DOIIdentifierException die) { // do we have to reserve DOI before we can
         * register it? if (die.getCode() == DOIIdentifierException.RESERVE_FIRST) {
         * this.reserveOnline(context, dso, identifier); connector.registerDOI(context,
         * dso, doi); } else { throw die; } }
         */
        // We have to reserve DOI before we can register it
        // DataCite MDS API changes (Eventual Consistency for read) is affecting the DOI
        // regsitration.
        // Check for reserved status are returning false even though reservation has
        // succeeded.
        // We can skip reservation status check - if we just reserved and the request
        // succeeded.
    ...
    public void updateMetadataOnline(Context context, DSpaceObject dso, String identifier)
        ...
        if (!ObjectUtils.equals(doiRow.getDSpaceObject(), dso)) {
            log.error(
                    "Refuse to update metadata of DOI {} with the metadata of "
        ...
        if (DELETED.equals(doiRow.getStatus()) || TO_BE_DELETED.equals(doiRow.getStatus())) {
            throw new DOIIdentifierException("You tried to update the metadata" + "of a DOI that is marked as DELETED.",
                    DOIIdentifierException.DOI_IS_DELETED);
        }
    ...
    public String mint(Context context, DSpaceObject dso) throws IdentifierException {
        ...
        try {
            ...
            log.error("Error while attemping to retrieve information about a DOI for "
                    ...
                    + ".");
            throw new RuntimeException("Error while attempting to retrieve " + "information about a DOI for "
                    + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID()
                    + ".", e);
        }
        if (null == doi) {
            try {
                ...
                log.error("Error while creating new DOI for Object of " + "ResourceType {} with id {}.", dso.getType(),
                        dso.getID());
                throw new RuntimeException("Error while attempting to create a " + "new DOI for "
                        + contentServiceFactory.getDSpaceObjectService(dso).getTypeText(dso) + " with ID " + dso.getID()
                        + ".", e);
            }
        }
    ...
    public void delete(Context context, DSpaceObject dso, String identifier) throws IdentifierException {
        ...
        if (null != doiRow) {
            if (!ObjectUtils.equals(dso, doiRow.getDSpaceObject())) {
                throw new DOIIdentifierException(
                        "Trying to delete a DOI out of " + "an object that is not addressed by the DOI.",
                        DOIIdentifierException.MISMATCH);
            }
        }
        ...
        try {
            ...
            throw new DOIIdentifierException("Not authorized to delete DOI.", ex,
                    DOIIdentifierException.UNAUTHORIZED_METADATA_MANIPULATION);
        } catch (SQLException ex) {
            log.error("SQLException occurred while deleting a DOI out of an item: " + ex.getMessage());
            throw new RuntimeException("Error while deleting a DOI out of the " + "metadata of an Item " + dso.getID(),
                    ex);
        }
    ...
    public void deleteOnline(Context context, String identifier) throws DOIIdentifierException {
        ...
        if (null == doiRow) {
            throw new DOIIdentifierException("This identifier: " + identifier + " isn't in our database",
        ...
        if (!TO_BE_DELETED.equals(doiRow.getStatus())) {
            ...
            throw new IllegalArgumentException("Couldn't delete this identifier:" + DOI.SCHEME + doiRow.getDoi()
     ...
     * @throws IdentifierException      If {@code identifier} is null or an empty
     *                                  string.
     * @throws IllegalArgumentException If the identifier couldn't be recognized as
     *                                  DOI.
     */
    public DSpaceObject getObjectByDOI(Context context, String identifier)
        ...
        if (doiRow.getDSpaceObject() == null) {
            ...
            throw new IllegalStateException(
                    "Found DOI " + doi + " in database, but no assigned Object could be found.");
        }
     ...
     *
     * @param context
     * @param dso     DSpaceObject to find doi for. DOIs with status TO_BE_DELETED
     *                will be ignored.
    ...
    /**
     * Load a DOI from the database or creates it if it does not exist. This method
     * can be used to ensure that a DOI exists in the database and to load the
     * appropriate TableRow. As protected method we don't check if the DOI is in a
     * decent format, use DOI.formatIdentifier(String) if necessary.
     * 
     * @param context
     * @param dso           The DSpaceObject the DOI should be loaded or created
     *                      for.
     * @param doiIdentifier A DOI or null if a DOI should be generated. The
     *                      generated DOI can be found in the appropriate column for
     *                      the TableRow.
            ...
            if (null != doi) {
                if (doi.getDSpaceObject() == null) {
                    ...
                    if (doi.getResourceTypeId() != null && doi.getResourceTypeId() != dso.getType()) {
                        ...
                        // reactivate it
                        throw new DOIIdentifierException("Cannot reassing " + "previously deleted DOI " + doiIdentifier
                                + " as the resource types of the object it was "
                                + "previously assigned to and the object it " + "shall be assigned to now divert (was: "
                                + Constants.typeText[doi.getResourceTypeId()] + ", trying to assign to "
                                + Constants.typeText[dso.getType()] + ").", DOIIdentifierException.DOI_IS_DELETED);
                    } else {
                        // reassign doi
                    ...
                    if (dso.getID().equals(doi.getDSpaceObject().getID())) {
                        ...
                        throw new DOIIdentifierException(
                                "Trying to create a DOI " + "that is already reserved for another object.",
