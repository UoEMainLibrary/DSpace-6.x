...
<xsl:stylesheet
    ...
    <xsl:template match="/">
        <oai_dc:dc xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/"
            ...
            <!-- date -->
            <xsl:for-each select="doc:metadata/doc:element[@name='dc']/doc:element[@name='date']">
                <dc:node><xsl:copy-of select="./node()"/></dc:node>
                <dc:name><xsl:value-of select ="local-name()"/></dc:name>
            </xsl:for-each>
            <!-- dc.date.issued -->
